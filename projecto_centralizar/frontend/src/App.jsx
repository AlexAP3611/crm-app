...
function ContactsPage() {
    const { contacts, total, loading, error, filters, updateFilter, setPage, refresh } = useContacts()
    ...
    const handleEnrich = async (service) => {
        ...
        try {
            ...
            if (service === 'Adscore') {
                const hasMissingDomain = resolvedContacts.some(c => !c.dominio || !c.dominio.trim())
                if (hasMissingDomain) {
                    setEnrichError('El dominio es obligatorio para enriquecer con Adscore')
                    setEnriching(null)
                    return
                }
                payload = {
            ...
            if (integration && integration.api_key && integration.api_key.trim()) {
                headers['Authorization'] = `Bearer ${integration.api_key.trim()}`
            }
