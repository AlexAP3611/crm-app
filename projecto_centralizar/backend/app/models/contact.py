...
class Contact(Base):
    ...
    sectors: Mapped[list["Sector"]] = relationship(  # noqa: F821
        secondary=contact_sectors, back_populates="contacts"
    )
    verticals: Mapped[list["Vertical"]] = relationship(  # noqa: F821
        secondary=contact_verticals, back_populates="contacts"
    )
    products_rel: Mapped[list["Product"]] = relationship(  # noqa: F821
        secondary=contact_products, back_populates="contacts"
    )
    cargos: Mapped[list["Cargo"]] = relationship(  # noqa: F821
        secondary=contact_cargos, back_populates="contacts"
    )
    campaigns: Mapped[list["Campaign"]] = relationship(  # noqa: F821
        secondary=contact_campaigns, back_populates="contacts"
    )
